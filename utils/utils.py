...
def number_to_money(number: float | int) -> str:
    """
...
def str_to_std_datetime(datetime_: str | datetime) -> Optional[datetime]:
    """
    Converts a string to a datetime object.

    Parameters:
        datetime_ (str | datetime): The datetime to be converted.
...
def normalize_string(s: str) -> str:
    """
    Lowercase and remove all non-alphanumeric characters for a tight matching criteria.
    """
    if not s:
        return ""
    return re.sub(r'[^a-z0-9]', '', s.lower())

def normalize_date(d_obj, fallback=None):
    import pandas as pd
    ...
    try:
        if hasattr(d_obj, 'date'):
            return pd.to_datetime(d_obj.date()).normalize()
        if isinstance(d_obj, date):
            return pd.to_datetime(d_obj).normalize()
        if isinstance(d_obj, datetime):
            return pd.to_datetime(d_obj.date()).normalize()
        if isinstance(d_obj, str):
            if len(d_obj) == 4:
                return pd.to_datetime(f"{d_obj}-01-01").normalize()
            return pd.to_datetime(d_obj.replace('Z', '+00:00'), format='mixed').normalize()
    except Exception:
        pass
    return fallback

def get_founder_name_to_id_map():
    """Extracts a centralized mapping of Founder names to their DB UUID strings."""
    from core.db import get_session, Founder
    session = get_session()
    try:
        db_founders = session.query(Founder.name, Founder.id).all()
        return {f.name: str(f.id) for f in db_founders}
    finally:
        session.close()
